...
/**
 * リポジトリ詳細ページ
 * 404エラーは not-found.tsx、その他のエラーは error.tsx でキャッチされる
 */
export default async function RepositoryPage({ params }: RepositoryPageProps) {
  ...
  try {
    ...
    // 404エラーの場合は not-found.tsx にリダイレクト
    if (error instanceof GitHubNotFoundError) {
      notFound();
    }
    // その他のエラーは error.tsx で処理されるように再スロー
    throw error;
  }
 ...
 * リポジトリヘッダーコンポーネント
 */
function RepositoryHeader({ repository }: { repository: GitHubRepository }) {
  return (
...
function RepositoryStats({ repository }: { repository: GitHubRepository }) {
  return (
    <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
      <StatCard icon="⭐" label="Stars" value={repository.stargazers_count} />
      <StatCard icon="👁️" label="Watchers" value={repository.watchers_count} />
      <StatCard icon="🍴" label="Forks" value={repository.forks_count} />
      <StatCard icon="🐛" label="Issues" value={repository.open_issues_count} />
    </div>
  );
}
 ...
 * リポジトリ詳細情報コンポーネント
 */
function RepositoryDetails({ repository }: { repository: GitHubRepository }) {
  return (
 ...
 * リポジトリアクションコンポーネント
 */
function RepositoryActions({ repository }: { repository: GitHubRepository }) {
  return (
    <div className="mt-8">
      <a
