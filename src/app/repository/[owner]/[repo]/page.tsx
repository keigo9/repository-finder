          ...
          <div className="rounded-lg border border-gray-200 bg-white p-8 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
              <StatCard
                icon="⭐"
                label="Stars"
                value={result.data.stargazers_count}
              />
              <StatCard
                icon="👁️"
                label="Watchers"
                value={result.data.watchers_count}
              />
              <StatCard
                icon="🍴"
                label="Forks"
                value={result.data.forks_count}
              />
              <StatCard
                icon="🐛"
                label="Issues"
                value={result.data.open_issues_count}
              />
            </div>
            ...
            <div className="space-y-4">
              ...
              <InfoRow label="作成日">
                {new Date(result.data.created_at).toLocaleDateString("ja-JP")}
              </InfoRow>

              <InfoRow label="最終更新">
                {new Date(result.data.updated_at).toLocaleDateString("ja-JP")}
              </InfoRow>
            </div>
