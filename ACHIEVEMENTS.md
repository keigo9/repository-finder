...
- **クライアント側のルータキャッシュ保持時間** (`next.config.ts:6-11`)
  - **staleTimes による二段階キャッシュ戦略**:
    - **サーバー側キャッシュ**: GitHub API レスポンスを5〜10分キャッシュ（上記の `revalidate`）
    - **クライアント側キャッシュ**: ページデータを5分間キャッシュ（`staleTimes.dynamic: 300`）
    - Next.js 15以降はデフォルトでルータキャッシュが0秒のため、明示的に設定が必要

  - **ページ遷移時のUX改善**:
    - **問題**: クライアント側キャッシュなしの場合、ページ遷移のたびに Suspense の fallback（スケルトン）が表示される
      - サーバー側キャッシュがヒットしても、クライアント側で再フェッチが発生
      - 「リポジトリ一覧 → 詳細 → 戻る」のたびにスケルトンが表示される
      - ユーザーは「データが読み込まれている」と感じてしまう

    - **解決**: staleTimes でクライアント側にもキャッシュを保持
      - 5分以内に同じページに戻ると、キャッシュから即座に表示
      - スケルトンが表示されず、ページが瞬時に表示される
      - ブラウザの戻る/進むボタンでもスムーズに遷移
      - 体感的なパフォーマンスが大幅に向上

  - **キャッシュ戦略の全体像**:
    1. **初回アクセス**: サーバーで GitHub API を呼び出し → サーバー側にキャッシュ → クライアント側にもキャッシュ
    2. **5分以内の再アクセス**: クライアント側キャッシュからスケルトンなしで即座に表示
    3. **5分経過後**: サーバー側キャッシュから取得（GitHub API は叩かない） → クライアント側キャッシュを更新
    4. **10分経過後**: サーバーが GitHub API を再フェッチ → 両方のキャッシュを更新
...
- **useTransition による UX 改善** (`components/ui/search-form.tsx:16-25`)
  - **問題**: 検索ボタンをクリックしても、データ取得完了まで何も反応がないように見える
  - **解決策**: React の `useTransition` フックを活用
    - 検索ボタンクリック時に即座に `isPending` 状態になる
    - ナビゲーション開始と同時にローディングUIを表示
    - ユーザーに「処理中」であることを明確に伝える

- **多層的なローディング表示** (`components/ui/search-form.tsx:39-48, 52-75`)
  1. **ボタンの状態変化**:
     - テキストが「検索」→「検索中...」に変化
     - `disabled` 属性で連打を防止
     - 視覚的にローディング中であることを表示
  ...
  2. **フォームの無効化**:
     - 検索フィールドも `disabled` になる
     - 検索中に入力内容を変更できないようにする
     - カーソルが `not-allowed` に変化

  3. **即座にスケルトンUIを表示**:
     - クリックした瞬間に検索結果エリアがスケルトンアニメーションに切り替わる
     - リポジトリカードと同じデザインのプレースホルダーを5つ表示
     - データ取得完了までの待ち時間が体感的に短く感じられる

- **ユーザー体験への効果**:
  - **従来**: ボタンを押しても無反応 → 「本当にクリックできた？」と不安になる
  - **改善後**: 即座に視覚的フィードバック → 安心して待てる
  - クリック成功の確信が得られ、UXが大幅に向上

### ページネーションクリック時の即時フィードバック

- **useTransition による UX 改善** (`components/ui/pagination.tsx:23-42`)
  - **問題**: ページネーションボタンをクリックしても、データ取得完了まで何も反応がない
    - 特に初めて訪れるページ（キャッシュがない場合）は遅く感じる
    - ユーザーが「クリックが反応していない」と誤解して連打してしまう

  - **解決策**: React の `useTransition` フックを活用
    - Link コンポーネントから button + onClick ハンドラーに変更
    - クリック時に `startTransition` でナビゲーションを開始
    - 即座に `isPending` 状態になり、全ボタンが無効化される

- **ボタンの無効化による視覚的フィードバック**
  - **全てのページネーションボタンを `disabled` に**:
    - 連打防止とユーザーの混乱を防ぐ
    - 視覚的にも `opacity-50` で無効状態を表現
    - カーソルが `not-allowed` に変化

- **ユーザー体験への効果**:
  - **従来**: ページネーションボタンをクリックしても反応なし → 不安でイライラする
  - **改善後**: クリック瞬間にボタンが無効化 → 処理中であることがわかる
  - 連打を防ぎ、快適なページ閲覧が可能に
